...
class Adam(Optimizer):
    ...
    def optimize(self):
        self.m = self.beta_1 * self.m + (1 - self.beta_1) * self.update.gradient
        self.v = self.beta_2 * self.v + (1 - self.beta_2) * np.power(self.update.gradient, 2)
        m_hat = self.m / (1 - np.power(self.beta_1, self.iteration))
        v_hat = self.v / (1 - np.power(self.beta_2, self.iteration))
        self.iteration += 1
        self.update.parameter -= self.learning_rate * m_hat / (np.sqrt(v_hat) + self.eps)
